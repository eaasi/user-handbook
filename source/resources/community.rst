.. Links to other docs

.. _community:

EAASI Community
*****************

.. _forum:

EAASI User Forum
===================

The `EAASI User <https://forum.eaasi.cloud/>`_ Discourse forum is not just for reporting bugs, but also a space for general discussion regarding emulation, software preservation, configuration workflows, assistance with using legacy software applications, and other topics related to the EAASI Research Allaince. Please sign up for an account today and join in the conversation!

.. _research_alliance:

EAASI Research Alliance
========================

SPN
===

For more information about the EAASI Research Alliance and our full slate of efforts and exclusibe benefits within the Software Preservation Network, please visit the Alliance page's on the `Software Preservation Network website <https://www.softwarepreservationnetwork.org/projects/emulation-as-a-service-infrastructure>`_!