...
.. note::
  ROM files are not provided along with emulators and templates in the open `emulation-as-a-service/emulators <https://gitlab.com/emulation-as-a-service/emulators>`_ repository because they contain proprietary Apple firmware. The EAASI team currently assesses that it cannot openly distribute these files under U.S. copyright law.
  
Importing a new or custom ROM file into EAASI in order to create or import a new Mac environment requires access to the top-level directory of Emulation-as-a-Service (``eaas-home``, using default installation configuration) on the EAASI server:

1. Copy the desired ROM file into the ``roms`` directory inside ``eaas-home``.

2. Log in to the demo UI. If you have a bootable Mac operating system installer (floppy, CD-ROM image), use the :ref:`make_new_environment`workflow to create a blank hard disk drive on which to install from scratch. Otherwise, all further steps related to ROM and emulator configuration are the same.
...
For more guidance on ROM files (including exactly what kind of ROM files/Mac models are appropriate for use with which emulators and operating systems), you may refer to sites like Emaculation or Macintosh Repository, or contact the Yale Software Preservation and Emulation team (softwarepreservation@yale.edu):
